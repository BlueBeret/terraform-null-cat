...
variable "create_resource_group" {
  type        = bool
  description = "Set to true to allow resource group creation by default"
  default     = true
}
...
variable "existing_resource_group_name" {
  ...
  default     = null # This allows for checking if an existing resource group name is provided

  validation {
    condition     = var.create_resource_group == true ? var.existing_resource_group_name == null : (var.existing_resource_group_name != null && var.existing_resource_group_name != "")
    error_message = "If create_resource_group is true, existing_resource_group_name must be null. If create_resource_group is false, existing_resource_group_name cannot be null or an empty string."
  }
}

variable "resource_group_suffix" {
  type        = string
  description = "(Optional) Provide the resource group suffix for the resource group to make it unique."
  default     = ""
  validation {
    condition     = var.resource_group_suffix != "" ? can(regex("^[a-z0-9-]+$", var.resource_group_suffix)) : true
    error_message = "resource_group_suffix name must contain only lowercase letters, numbers, and hyphens."
  }
}

variable "scp_azure_gxp_relevant" {
  type        = bool
  description = "(Required) GXP Relevant or NON-GXP. allowed values are 'true' or 'false'"
  validation {
    condition     = contains([true, false], var.scp_azure_gxp_relevant)
    error_message = "Value of scp_azure_gxp_relevant must be either true or false"
  }
}
...
variable "workload_name" {
  type        = string
  description = "(Required) Provide the workload name for naming convention."
}

variable "prevent_deletion_if_contains_resources" {
  type        = bool
  default     = false
  description = "(Optional) control deletion of resource group when other resources exist in the resource group."
  nullable    = false
}

variable "workflow_id" {
  type        = string
  description = "(Required) The StackGuardian workflow ID. This value comes from the system variable and passed through TF_VAR_workflow_id"
}

variable "stage" {
  type        = string
  description = "Provide the stage for naming convention. Allowed values are prod, dev, test, qas, sbx."
  validation {
    condition     = contains(["prod", "dev", "test", "qas", "sbx"], var.stage)
    error_message = "Stage must be either prod, dev, test, qas, or sbx."
  }
}
...
variable "counter" {
  description = "A counter to ensure unique resource names when multiple instances are created."
  type        = number
  default     = 1
  validation {
    condition     = var.counter >= 0 && var.counter <= 999
    error_message = "counter must be between 1 and 999."
  }
}

variable "suffix" {
  type        = string
  description = "(Optional) Provide the suffix to extend the names of the resources."
  default     = ""
  validation {
    error_message = "Suffix name must contain only lowercase letters, numbers."
    condition     = var.suffix != "" ? can(regex("^[a-z0-9]+$", var.suffix)) : true
  }
}

variable "create_lock" {
  type        = bool
  description = "(Optional) Create resource locks. Default to true."
  default     = false
}

variable "lock_type" {
  type        = string
  description = "(Optional) The type of lock to apply to the resources. Possible values are 'CanNotDelete' and 'ReadOnly'. Default to CanNotDelete."
  default     = "CanNotDelete"

  validation {
    condition     = var.lock_type != null ? contains(["CanNotDelete", "ReadOnly"], var.lock_type) : true
...
variable "create_log_analytics_workspace" {
  type        = bool
  description = "(Optional) Create Log Analytics Workspace for Container App Environment. Default to true."
  default     = true
}
variable "law_sku" {
  type        = string
  description = "(Optional) Specifies the SKU of the Log Analytics Workspace. Possible values are PerGB2018, PerNode, Premium, Standalone, Standard, CapacityReservation, LACluster and Unlimited. Defaults to PerGB2018."
  default     = "PerGB2018"
}
...
variable "law_local_authentication_enabled" {
  type        = bool
  description = "(Optional) Specifies if the log Analytics workspace should allow local authentication methods in addition to Microsoft Entra (Azure AD). Defaults to true."
  default     = true
}
variable "law_cmk_for_query_forced" {
  type        = bool
...
variable "log_analytics_workspace_id" {
  type        = string
  description = "(Optional) The ID of an existing Log Analytics Workspace. Required if create_log_analytics_workspace is false."
  default     = null

  validation {
    condition     = var.create_log_analytics_workspace == false ? (var.log_analytics_workspace_id != null && var.log_analytics_workspace_id != "") : true
    error_message = "If create_log_analytics_workspace is false, log_analytics_workspace_id cannot be null or an empty string."
  }
}
...
variable "create_container_app_environment" {
  type        = bool
  description = "(Optional) Create Container App Environment. Default to true."
  default     = true
}
variable "container_app_environment_id" {
  type        = string
  description = "(Optional) The ID of an existing Container App Environment. Required if create_container_app_environment is false."
  default     = null

  validation {
    condition     = var.create_container_app_environment == false ? (var.container_app_environment_id != null && var.container_app_environment_id != "") : true
    error_message = "If create_container_app_environment is false, container_app_environment_id cannot be null or an empty string."
  }
}
variable "cae_dapr_application_insights_connection_string" {
  type        = string
  description = " (Optional) Application Insights connection string used by Dapr to export Service to Service communication telemetry. Changing this forces a new resource to be created."
  default     = null
}
variable "cae_internal_load_balancer_enabled" {
  type        = bool
...
variable "cae_mutual_tls_enabled" {
  type        = bool
  description = " (Optional) Should mutual transport layer security (mTLS) be enabled? Defaults to false."
  default     = false
}
variable "cae_public_network_access" {
  type        = string
  description = " (Optional) The public network access setting for the Container App Environment. Possible values are Enabled and Disabled. Defaults to Disabled."
  default     = "Disabled"

  validation {
    condition     = contains(["Enabled", "Disabled"], var.cae_public_network_access)
    error_message = "cae_public_network_access must be either 'Enabled' or 'Disabled'."
  }
}
variable "cae_infrastructure_subnet_id" {
  type        = string
  description = "(Optional) The ID of the subnet to deploy the Container App Environment into."
  default     = null
}
...
variable "cae_workload_profile_id" {
  type = object({
    name                  = string
...
EOT
  ...
  validation {
    condition = (
      var.cae_workload_profile_id == null ? true :
      (
        can(var.cae_workload_profile_id.name) &&
        can(var.cae_workload_profile_id.workload_profile_type) &&
        contains(
          ["Consumption", "D4", "D8", "D16", "D32", "E4", "E8", "E16", "E32"],
          var.cae_workload_profile_id.workload_profile_type
        )
      )
    )
    error_message = "If cae_workload_profile_id is provided, both name and workload_profile_type must be set, and workload_profile_type must be one of 'Consumption', 'D4', 'D8', 'D16', 'D32', 'E4', 'E8', 'E16', or 'E32'."
  }
}

variable "cae_identity" {
  type = object({
    identity_ids = optional(set(string))
    type         = string
  })
  description = <<-EOT
 - `type` - (Required) Specifies the type of Managed Service Identity that should be configured on this resource. Possible values are `SystemAssigned`, `UserAssigned`, `SystemAssigned, UserAssigned` (to enable both).
 - `identity_ids` - (Optional) Specifies a list of User Assigned Managed Identity IDs to be assigned to this resource.
EOT
  default     = null

  validation {
    condition = (
      var.cae_identity == null ? true :
      (
        (var.cae_identity.type == "UserAssigned" || var.cae_identity.type == "SystemAssigned, UserAssigned")
        ? (
          can(var.cae_identity.identity_ids) &&
          var.cae_identity.identity_ids != null &&
          length(var.cae_identity.identity_ids) > 0
        )
        : true
      )
    )
    error_message = "If `identity.type` is set to `UserAssigned` or `SystemAssigned, UserAssigned`, then `identity.identity_ids` must be set and non-empty."
  }
}
...
variable "create_private_endpoint" {
  type        = bool
  description = "Flag to create private endpoint for the Azure Container Apps. Default to false."
  default     = false
}
variable "private_endpoint_subnet_id" {
  type        = string
  default     = null
  description = "(Optional) The ID of the Subnet within which the Private Endpoint should be created. Changing this forces a new resource to be created."

  validation {
    condition     = !(var.create_private_endpoint == true && (var.private_endpoint_subnet_id == null || var.private_endpoint_subnet_id == ""))
    error_message = "If 'create_private_endpoint' is true, 'private_endpoint_subnet_id' must be provided and cannot be an empty string."
  }
}
...
> Note: This is only used if the `create_private_endpoint` variable is set to true.
DESCRIPTION
}
...
- `role_definition_name` - (Required) The ID or name of the role definition to assign to the principal.
